...
export interface ActivityLine { itemId: string; kind: string; summary: string; detail: string | null; ok: boolean | null; ms: number | null }
...
const upsert = (lines: ActivityLine[], line: ActivityLine) =>
  lines.some((a) => a.itemId === line.itemId) ? lines.map((a) => (a.itemId === line.itemId ? { ...a, ...line, kind: line.kind || a.kind, summary: line.summary || a.summary } : a)) : [...lines, line];
...
export function apply(view: RunView, e: RunEvent & { at?: number }): RunView {
  switch (e.type) {
    ...
    case "item.started": {
      const line = { itemId: e.itemId, kind: e.kind, summary: e.summary, detail: null, ok: null, ms: null };
      const t = currentTurn(view);
      return withTurn({ ...view, activity: upsert(view.activity, line) }, { ...t, activity: upsert(t.activity, line) });
    }
    case "item.completed": {
      const line = { itemId: e.itemId, kind: "", summary: e.summary, detail: e.detail, ok: e.ok, ms: e.ms };
