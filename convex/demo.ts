...
async function seed(ctx: MutationCtx, userId: Id<"users">) {
  ...
  const say = (chatId: Id<"chats">, author: string, text: string, at: number, extra: Partial<{ kind: string; runId: Id<"runs">; turn: number; reactions: { emoji: string; by: string[] }[] }> = {}) =>
  ...
  async function run(chatId: Id<"chats">, agentId: Id<"agents">, harness: string, model: string, effort: string, dispatch: Id<"messages">, start: number, steps: [string, string, number][], reply: string, landing: unknown, branch: string | null) {
    const runId = await ctx.db.insert("runs", { chatId, agentId, runnerId, dispatchedBy: DEMO_LOGIN, dispatchMessageId: dispatch, state: "landed", branch, worktree: null, resumeCursor: null, landing, startedAt: start, endedAt: start + 4 * min,
      execution: { model, modelName: model, effort, accountOwner: DEMO_LOGIN, accountEmail: null, accountPlan: null, machineName: "demo-mac" } });
    const events: Record<string, unknown>[] = [{ type: "turn.started", runId, turnId: "t1", at: start + 2_000 }];
    let at = start + 4_000;
    steps.forEach(([kind, summary, ms], i) => {
      events.push({ type: "item.started", runId, itemId: `i${i}`, kind, summary, at });
      events.push({ type: "item.completed", runId, itemId: `i${i}`, summary, detail: null, ok: true, ms, at: at + ms });
      at += ms + 1_500;
    });
    const replyAt = at + 1_000;
    events.push({ type: "turn.completed", runId, turnId: "t1", at: replyAt + 500 });
    for (const [seq, event] of events.entries()) await ctx.db.insert("runEvents", { runId, seq, event });
    await say(chatId, `agent:${agentId}`, reply, replyAt, { kind: "report", runId, turn: 1 });
    void harness;
    return runId;
  }
  ...
  await run(tracker, claude, "claude", "Fable 5.1", "high", d1, now - 38 * min,
  ...
  await run(copy, codex, "codex", "GPT-6 Astra", "medium", d2, now - 11 * min, [["read", "Read src/components/Empty.tsx", 250]],
