...
const cases = [
  ...
  // long messages
  { name: "long status update to the team", agents: ONE, t: [], m: C("apek", "quick update on where things are. the auth work landed yesterday and claude's branch for the session store is up but i haven't reviewed it yet, will do that after lunch. the migration is blocked on the staging db being down, i pinged infra. for friday i think we're fine on the api side, the frontend is the risk. noah if you have cycles the settings page still has the layout bug on narrow screens. i'll be out thursday afternoon."), want: null },
  { name: "long technical paragraph to a person", agents: ONE, t: [], m: C("apek", "noah, about the rate limiter: the way i see it the token bucket refills on a timer in the worker, and when redis is down we fall through to the in-memory map which is per-process, so behind three replicas you effectively get 3x the limit. that's probably fine for now but it means the tests that assert exact counts are going to be flaky in ci where we run two workers. i'd rather relax the assertions than add a lock. what do you think? i can pair on it tomorrow morning if you want."), want: null },
  { name: "long pasted code for humans", agents: ONE, t: [], m: C("noah", "here's the bit i was talking about, for context:\n\nexport async function refill(key: string) {\n  const now = Date.now();\n  const bucket = (await redis.get(key)) ?? { tokens: MAX, at: now };\n  bucket.tokens = Math.min(MAX, bucket.tokens + (now - bucket.at) * RATE);\n  bucket.at = now;\n  await redis.set(key, bucket);\n  return bucket;\n}\n\nnote the get/set isn't atomic, that's the race apek mentioned. not asking anyone to change it yet, just want us to agree on whether it matters before the release."), want: null },
  { name: "long message ending with an ask", agents: ONE, t: [], m: C("apek", "ok so after digging through the logs from last night: the 502s line up exactly with the worker restarts, and the restarts happen because the healthcheck times out during the cache warm. the warm takes ~40s on a cold pod because it loads every tenant's config sequentially. i think the fix is to warm lazily per tenant on first request instead of everything at boot, and bump the healthcheck grace period as a stopgap. can you make both changes on a branch so we can look at the diff?"), want: "claude" },
  { name: "long recap mentioning the agent's past work", agents: ONE, t: [C("apek", "@claude add subtract"), A("claude", "Done, pushed to beam/add-subtract.")], m: C("noah", "for anyone catching up: claude added the subtract helper this morning and it's on beam/add-subtract, i reviewed it and it's fine. the bigger question is whether we keep math.ts as a grab bag or split it by domain before it grows. i lean towards splitting once we have more than ten helpers, which is not today. no action needed, just flagging so we don't get surprised later."), want: null },
  { name: "long question to a person with a code question inside", agents: ONE, t: [], m: C("apek", "noah when you get a sec: i'm writing the postmortem for the deploy and i want to get the timeline right. you were the one who noticed the queue backing up, was that before or after the rollback? also do you remember whether the alert fired at all, because i can't find it in pagerduty. and one more thing, did we ever confirm the retry logic in the consumer actually retries on 5xx or only on timeouts? i can check the code myself later if you don't remember."), want: null },
  { name: "long spec handed to the agent by name, no @", agents: ONE, t: [], m: C("noah", "claude, here's the full spec for the export feature so you have everything in one place. users pick a date range and a format (csv or json). the export runs in a background job because big tenants have millions of rows, and the user gets an email with a signed download link that expires in 24h. the job should be resumable so a crash doesn't restart from zero. put the job in src/jobs/export.ts, reuse the existing mailer, and add a migration for an exports table that tracks status. don't build the ui yet, just the backend and tests."), want: "claude" },
  { name: "long message to a person that could be misread as a task", agents: ONE, t: [], m: C("apek", "noah, for the onboarding doc you're writing: the steps are clone the repo, run pnpm install, copy .env.example to .env.local, run the dev server, and then log in with the seeded admin user. the seed script is at scripts/seed.ts and it's idempotent so it's safe to run twice. mention that the first run takes a while because it builds the native deps. i'd also add a troubleshooting section for the port conflict on 3000 since everyone hits that."), want: null },
