...
export function PersonAvatar({ login, name, image, className = "", hue }: { login: string; name?: string; image?: string | null; className?: string; hue: string }) {
  if (image) return <img className={`av ${className}`} src={image} alt={name ?? login} style={{ objectFit: "cover" }} />;
  return <span className={`av ${hue} ${className}`} title={name ?? login}>{initials(name ?? login)}</span>;
}
