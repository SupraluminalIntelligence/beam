...
export function CadWorkspace({ reference, resolve, choose, items, scope, setScope, attach, uploading }: Props) {
  ...
  return <div className={`cad-workspace${drop.dragging ? " dragging" : ""}`} {...drop.handlers}>
    <header className="cad-heading"><div><span className="workspace-eyebrow">ENGINEERING</span><h2>CAD Viewer</h2></div><div><button className="btn ghost" onClick={() => { choose(null); void load(async () => ({ name: "Flanged reducer", bytes: new ArrayBuffer(0) }), null, true); }}>Sample model</button><button className="btn ghost" onClick={() => setLibrary(!library)} aria-expanded={library}>From Context</button><button className="btn" onClick={() => input.current?.click()}>Open model</button></div></header>
    ...
    {library && <section className="cad-library" aria-label="Models in Context"><div className="context-scopes" role="group" aria-label="Model source scope"><button aria-pressed={scope === "chat"} onClick={() => setScope("chat")}>This chat</button><button aria-pressed={scope === "workspace"} onClick={() => setScope("workspace")}>Workspace</button></div><input className="workspace-search" placeholder="Find a model…" aria-label="Find a model" value={search} onChange={e => setSearch(e.target.value)} /><div className="cad-library-list">{shown?.map(item => <button key={item.key} onClick={() => { if (JSON.stringify(item.reference) === referenceKey) void load(signal => resolve(item.reference, signal)); else choose(item.reference); }}><span>◇</span><b>{item.name}</b><span>Open ↗</span></button>)}{shown?.length === 0 && <p>No STEP, IGES, STL, or OBJ models in {scope === "chat" ? "this chat" : "workspace sources"}.</p>}{!shown && <p>Loading models…</p>}</div><small>Viewing a workspace model doesn’t add it to this chat’s context.</small></section>}
    ...
    {model && local && <div className="cad-local-note"><span>Local preview · only on this device</span><button disabled={uploading} onClick={() => void attach([local])}>{uploading ? "Attaching…" : "Attach to chat"}</button></div>}
    {drop.dragging && <div className="cad-drop-overlay">Drop a model to open it</div>}
  </div>;
}
